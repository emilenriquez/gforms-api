...
module.exports = class UserResponseService extends Schmervice.Service {
  async getResponsesByForm(formId, loggedInUser) {
    try {
      const {
      ...
      if(loggedInUser.role === 'USER') {
        const userResponse = await UserResponseModel.query().where(
          {
        ...
        if(!userResponse.length) {
          throw Boom.forbidden('you are not allowed to see the response for this form!')
        }

        return userResponse;
      }

      const userResponses = UserResponseModel.query();
  ...
  async getAll(userCredentials) {
    try {
      const {
        UserResponse: UserResponseModel,
      } = this.server.models();

      // only admin can see all the responses
      if(userCredentials.role !== 'ADMIN') {
        throw Boom.forbidden('you are not allowed to see all the responses!')
      }

      const userResponses = await UserResponseModel.query();
